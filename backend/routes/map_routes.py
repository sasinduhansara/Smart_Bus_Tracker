...
from services.passenger_schedule_service import (
    PassengerScheduleError,
...
def passenger_search_stops():
    ...
    return jsonify({
        "status": "success",
        "query": query,
        "stops": search_public_stops(query, limit=limit),
    })
...
def passenger_search_routes():
    ...
    except PassengerScheduleError as error:
        return jsonify({
            "status": "error",
            "error": error.message,
            "code": error.code,
...
def passenger_search_services():
    route_identifier = str(
        request.args.get("routeId")
        or request.args.get("routeNumber")
        or ""
    ).strip()
    ...
    if not route_identifier:
        return jsonify({
            "status": "error",
            "error": "routeId or routeNumber is required",
            "services": [],
        }), 400

    try:
        result = get_public_timetable(
            route_identifier,
            service_date=str(
                request.args.get("date") or ""
            ).strip(),
...
@map_bp.route(
    "/api/passenger/routes/<route_id>/timetable",
    methods=["GET"],
)
def passenger_route_timetable(route_id: str):
    try:
        result = get_public_timetable(
            route_id,
            service_date=str(
